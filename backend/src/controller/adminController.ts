...
import { orderItems, products, users } from "../db/schema.js";
import { desc, eq } from "drizzle-orm";
import { getLocalUser } from "../db/user.js";
import { isAdmin } from "../db/role.js";
...
export const getImagekitAuth = async (req:Request,res:Response,next:NextFunction) => {
    try {
...
export const updateAdminProduct = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const [row] = await db.update(products).set(data).where(eq(products.id,req.params.id as string))
        .returning();
