...
function OrderVideoPage() {
    const {call,client,error,id,isLoading,loadError,order,paid} =  useOrderVideoPage()
    ...
    if(loadError || !order){
        return (
            <PageError
             message="Order not found or you dont have access"
  ...
  return (
    <div className='space-y-4 text-left'>
        <Link className="btn btn-ghost btn-sm gap-2 text-base-content/80">
               <ArrowLeftIcon className='size-4' aria-hidden />
               Back to support chat     
        </Link>
        <div className='card border border-base-300 bg-base-300 shadow-sm'>
            ...
            <p className='text-sm text-base-content/70'>
                 Same room as the invite link in chat, Allow camera and microphone when your broswer asks.   
            </p>
               </div>
            </div>
        </div>

        <div className='flex min-h-130 flex-col overflow-hidden rounded-box border-box border border-base-300 bg-base-100'>
