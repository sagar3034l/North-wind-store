...
function OrderDetailPage() {
  ...
  return (
    <div className='space-y-8 text-left'>
        <Link to="/orders" className='btn btn-ghost btn-sm gap-2 px-0 text-base-content/70 hover:text-primary'>
        ...
        <div className='overflow-hidden rounded-xl border border-base-300 bg-base-100'>
            <div className='bg-linear-to-br from-primary/12 via-base-100 to-base-200/90 px-5 py-6 sm:px-8 sm:py-8'>
                <div className='flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between'>
                    <div>
                        <p className='text-xs font-semibold upparcase tracking-wider text-primary'>
                                Order Details
                        </p>
                        ...
                        <p className='mt-2 text-sm text-base-content/78'>
                    ...
                    <div className='flex flex-col gap-3 border-t border-base-300/80 pt-4 lg:border-t-0 lg:pt-0 lg:text-right'>
                       ...
                       <div>
                         <p className='text-xs font-medium uppercase tracking-wide text-base-content/50'>Order total</p>
                         <p className='text-2xl font-bold tabular-nums text-base-content sm:text-3xl'>{formatPrice(order.totalCents,"inr")}</p>
                       </div>
                    </div>
                </div>    
            </div>
            <div className='border-t border-base-300 bg-base-200/40 px-5 py-4 sm:px-8'>
                <p className='max-w-3xl text-sm leading-relaxed text-base-content/80'>
                   ...
                   <strong>Support chat</strong> tab after payment. video call links are shared in that thread; everyone joins with the same link
                </p>     
            </div>
        </div>
        <div>
           <div className='flex items-center gap-7 border-b border-base-300 pb-3'>
           ...
           </div>
          {
            !paid ? (
              <div role='alert' className='alert alert-warning mt-4 text-sm'>
                 ...
                 <span>
                   Support unlocks when the order is marked {" "}
                   <strong>paid</strong> (once payment is confirmed)
                 </span>
              </div>
            ):null
          }
