...
export async function uploadImageToImageKit(file, getToken, opts = {}) {
  ...
  let auth;
  try {
    auth = await apiFetch("/api/admin/imagekit/auth", { getToken });
  } catch (error) {
    if (error instanceof Error && (error.message === "Unauthorized" || error.message === "Admin only")) {
      throw new Error("ImageKit auth failed: you are not signed in as an admin.");
    }
    throw error;
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(
      typeof data?.message === "string"
        ? `ImageKit upload failed: ${data.message}`
